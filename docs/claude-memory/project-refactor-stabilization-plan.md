...
**Known env quirk:** a local venv may be missing `grapheme` (declared in requirements.txt — CI installs it), which fails one Threads caption test locally; fix is `pip install -r requirements.txt`. Root pytest.ini now sets `pythonpath = .`, so tests run from repo root without PYTHONPATH juggling.
