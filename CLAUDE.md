...
- **DOCS** | For documentation updates and improvements
- **REFACTOR** | For code restructuring and architectural changes
- **INIT** | For initial file uploads

Format: `[PREFIX] | [Description]`
Example: `UPDATE | Add image retry mechanism for Flickr URL validation`
Example: `DOCS | Update README with Git-based state management architecture`

### Commit Attribution
Always include proper attribution in commit messages:
```
...
When commenting on GitHub Issues using Claude Code:
- **Always add attribution**: End all issue comments with: `*Written with Claude Code assistance*`
- **Consistent Format**: Use a horizontal rule separator (`---`) before the attribution line
- **Standard Practice**: This ensures transparency about AI assistance in project communication