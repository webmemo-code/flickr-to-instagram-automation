...
- `WORDPRESS_USERNAME` - WordPress username for authenticated content access
- `WORDPRESS_APP_PASSWORD` - WordPress Application Password for API authentication
- `PERSONAL_ACCESS_TOKEN` - GitHub Personal Access Token with repo scope for repository variable write access
...
For WordPress API failure alerts, configure these SMTP settings as GitHub Secrets:

- `NOTIFICATION_EMAIL` - Email address to receive API failure alerts (e.g., walter@webmemo.ch)
- `SMTP_HOST` - SMTP server hostname (default: smtp.gmail.com)
- `SMTP_PORT` - SMTP server port (default: 587)
- `SMTP_USERNAME` - SMTP authentication username (usually your email address)
- `SMTP_PASSWORD` - SMTP authentication password (use App Password for Gmail)

**Email Alert Features:**
- Automatic notifications when WordPress API access fails due to bot protection
- Detailed error reports with troubleshooting steps for Cloudflare settings
- Professional HTML and text email formats with investigation guidance
- Graceful degradation - automation continues without email if SMTP not configured
...
- **Security**: GitHub Secrets for credential management, PERSONAL_ACCESS_TOKEN for state management
- **Authentication**: PERSONAL_ACCESS_TOKEN with repo scope required for repository variable write access (built-in GITHUB_TOKEN lacks repository variable write permissions)
...
- **Blog Content**: Authenticated WordPress content extraction from travelmemo.com and reisememo.ch
...
3. Extracts full blog content using WordPress authentication
4. Generates AI caption using OpenAI GPT-4 Vision with enhanced blog context
...
## Recent Changes
- **ARCHITECTURE SIMPLIFICATION**: Eliminated redundant LAST_POSTED_POSITION variables, now derive position from INSTA_POSTS array for single source of truth (September 2025)
- **WORDPRESS AUTHENTICATION**: Added WordPress Application Password authentication for full blog content access (September 2025)
- **ENVIRONMENT NAMING**: Standardized environment names to `primary-account` and `secondary-account` for generic reusability (September 2025)
- **BLOG CONTENT ENHANCEMENT**: WordPress authenticated content extraction provides rich editorial context for AI caption generation (September 2025)
- **TOKEN CLARIFICATION**: Confirmed PERSONAL_ACCESS_TOKEN is required for repository variable write access (built-in GITHUB_TOKEN lacks required permissions) (September 2025)
...
**TOKEN PERMISSIONS VERIFIED**: PERSONAL_ACCESS_TOKEN confirmed as required for repository variable write access ✅
...
**Repository Variables with Environment Prefixes** (per-environment isolation):
- `PRIMARY_ACCOUNT_FAILED_POSITIONS_{album_id}` - Failed positions for retry
- `PRIMARY_INSTA_POSTS_{album_id}` - Complete Instagram post audit trail (includes position tracking)
- `SECONDARY_ACCOUNT_FAILED_POSITIONS_{album_id}` - Reisememo failed positions
- `SECONDARY_INSTA_POSTS_{album_id}` - Reisememo audit trail (includes position tracking)
...
- **GitHub Token**: ✅ VERIFIED (PERSONAL_ACCESS_TOKEN required for repository variable write access)
- **Photo Progression**: ✅ WORKING (no more repetition)
- **Workflow schedule**: 📅 Daily at 18:13 UTC (20:13 CEST)

**Last Successful Test**: September 15, 2025 - PERSONAL_ACCESS_TOKEN permissions verified as required
...
## GitHub Token Permission Requirements (September 2025)

### Issue #148: Repository Variable Write Access Requirements
**Problem**: Built-in GITHUB_TOKEN lacks permissions to write to GitHub Repository Variables across workflow runs.

**Root Cause Analysis**:
1. **Token Limitation**: Built-in `GITHUB_TOKEN` cannot write to GitHub Repository Variables
2. **Persistence Requirement**: Variables like `LAST_POSTED_POSITION_{album_id}` must be persisted between workflow runs
3. **State Management**: Without persistent state, automation would repeat the same photo
4. **Permission Scope**: Only Personal Access Tokens with `repo` scope can modify repository variables
5. **Environment Variable Limitation**: GitHub Environments cannot be written to from workflows (attempted but failed)

**Solution Confirmed**:
1. **PERSONAL_ACCESS_TOKEN Required**: Must use Personal Access Token with `repo` scope for repository variable write access
2. **Built-in Token Limitation**: `GITHUB_TOKEN` only has read access to repository variables
3. **Architecture Decision**: PERSONAL_ACCESS_TOKEN is essential for state persistence functionality
4. **Repository Variables Only**: State stored in Repository Variables with environment-specific prefixes (not Environment Variables)

**Technical Implementation**:
- All workflows use `GITHUB_TOKEN: ${{ secrets.PERSONAL_ACCESS_TOKEN }}` for repository variable write access
- Personal Access Token configured with `repo` scope in repository secrets
- State variables stored as Repository Variables with environment prefixes for isolation
- Environment-specific prefixes: `PRIMARY_ACCOUNT_*` and `SECONDARY_ACCOUNT_*`

**Key Insight**: Repository Variables with environment prefixes provide multi-account isolation:
- `PRIMARY_INSTA_POSTS_{album_id}`: Primary account complete audit trail with position tracking
- `SECONDARY_INSTA_POSTS_{album_id}`: Secondary account complete audit trail with position tracking

**Result**: PERSONAL_ACCESS_TOKEN enables repository variable write access, allowing automation to progress through album photos (1 → 2 → 3, etc.) correctly using simplified position tracking via Instagram posts array.
...
For all commits with Walter as author, include both co-authors as follows:
